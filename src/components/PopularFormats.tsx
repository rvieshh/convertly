      ...
      <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {POPULAR.map((p, idx) => (
          <motion.a
            ...
            whileHover={{ y: -3 }}
            className="group flex items-center justify-center gap-2 rounded-xl border border-zinc-800 bg-zinc-900/50 px-4 py-4 text-sm font-semibold transition-colors hover:border-primary/50 hover:bg-zinc-900"
          >
            <span className="text-zinc-300">{p.from}</span>
            <ArrowRight className="h-3.5 w-3.5 text-primary transition-transform group-hover:translate-x-0.5" />
            <span className="text-primary">{p.to}</span>
          </motion.a>
        ))}
      </div>
    </section>
